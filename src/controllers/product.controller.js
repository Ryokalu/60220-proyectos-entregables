...
export const add_product = async (req, res) => {
    try {
        ...
        if (!empty) {
            ...
            res.send({ status: "success", payload: status })
        }
        else {
        ...
        if (e.code === 11000) {
            res.status(401).send("Codigo repetido, Favor de utilizar otro")
        }
        else if (e.name === "error al Crear el producto") {
            res.status(401).send("Campos Vacios, favor de revisar Vacios")
        }
        else {
            console.log(e)
            res.status(500).send("un error ha ocurrido")
...
export const delete_product = async (req, res) => {
    try {
        ...
        if (prod.owner !== "admin") {
            ...
            // const user = userManager.findOne({ email: prod.owner })

            emailProvider.deleteProdEmail(prod[0].title)
        }
