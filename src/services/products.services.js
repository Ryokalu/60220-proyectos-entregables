...
export default class ProdServices {
    ...
    getProducts = async (page, limit, find, url, ord) => {
        ...
        item.prevLink = item.hasPrevPage ? `${link}/products?page=${item.prevPage}` : '';
        item.nextLink = item.hasNextPage ? `${link}/products?page=${item.nextPage}` : '';
